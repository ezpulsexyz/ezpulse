    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="w-full max-w-2xl overflow-hidden rounded-3xl bg-white shadow-2xl">
        <div className="flex items-center justify-between border-b px-8 py-6">
          <div>
            <h2 className="text-2xl font-semibold">Write Investor Thesis</h2>
            <p className="text-sm text-zinc-500">for ${tokenSymbol}</p>
          </div>
          <button
            type="button"
            onClick={onClose}
            className="text-2xl text-zinc-400 transition hover:text-zinc-600"
            aria-label="Close"
          >
            ×
          </button>
        ...
        <div className="space-y-6 p-8">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Explain why you believe this token will perform well (or not)..."
              className="h-40 w-full resize-y rounded-2xl border border-zinc-200 p-4 focus:border-blue-400 focus:outline-none"
            />
          </div>
          ...
          <div>
            ...
            <div className="mb-3 flex gap-2">
              <input
                ...
                className="flex-1 rounded-2xl border border-zinc-200 px-4 py-2.5"
              />
              <button
                type="button"
                onClick={handleAddPoint}
                className="rounded-2xl bg-zinc-900 px-5 font-medium text-white"
              >
                Add
              </button>
            </div>
            ...
            {keyPoints.length > 0 && (
              <div className="space-y-2">
                {keyPoints.map((point, index) => (
                  <div
                    key={index}
                    className="flex items-center justify-between rounded-2xl bg-zinc-50 px-4 py-2 text-sm"
                  >
                    <span>{point}</span>
                    <button
                      type="button"
        ...
        <div className="flex justify-end gap-3 border-t px-8 py-6">
          <button
            type="button"
            onClick={onClose}
            className="rounded-2xl border border-zinc-200 px-8 py-3 font-medium"
          >
            Cancel
          </button>
          <button
            ...
            className="rounded-2xl bg-emerald-600 px-8 py-3 font-medium text-white disabled:bg-zinc-300 disabled:text-zinc-500"
          >
