...
async function rpcCall(method: string, params: unknown[]): Promise<Rec | null> {
  for (const url of SOLANA_RPCS) {
    try {
      ...
      if (data.result !== undefined) return data.result as Rec;
    } catch { /* try next RPC */ }
  }
  return null;
}

/** Fetch a wallet's raw SPL token balances: mint → uiAmount. Read-only, no signature. */
...
export async function fetchPortfolio(owner: string, feed: LiveLaunch[]):
  Promise<{ holdings: Holding[]; totalUsd: number; scanned: number } | null> {
  const balances = await fetchTokenBalances(owner);
