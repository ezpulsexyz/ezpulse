...
interface Snap {
  ca: string; symbol: string; price: number; mcap: number; liquidity: number;
  volume24h: number; volume1h: number; change24h: number;
  buys24h: number; sells24h: number; buys1h: number; sells1h: number;
  holders: number | null; curve: number | null; graduated: boolean;
}
  ...
  for (const url of [
    ...
    try {
      ...
      for (const pool of pools) {
        ...
        if (!ca) continue;
        const s24 = a.stats24h as Rec | undefined, s1 = a.stats1h as Rec | undefined;
        found.set(ca.toLowerCase(), {
          ca, symbol: String(a.symbol ?? "?"),
          price: num(a.usdPrice), mcap: num(a.mcap) || num(a.fdv),
          liquidity: num(a.liquidity), volume24h: num(s24?.buyVolume) + num(s24?.sellVolume),
          volume1h: num(s1?.buyVolume) + num(s1?.sellVolume), change24h: num(s24?.priceChange),
          buys24h: num(s24?.numBuys), sells24h: num(s24?.numSells),
          buys1h: num(s1?.numBuys), sells1h: num(s1?.numSells),
    ...
    try {
      ...
      if (res.ok) {
        ...
        for (const p of pairs) {
          ...
          const pc = p.priceChange as Rec | undefined, vol = p.volume as Rec | undefined;
          const tx = p.txns as Rec | undefined, t24 = tx?.h24 as Rec | undefined, t1 = tx?.h1 as Rec | undefined;
          found.set(ca.toLowerCase(), {
            ca, symbol: String(base?.symbol ?? "?"),
            price: p.priceUsd ? parseFloat(String(p.priceUsd)) : 0,
            mcap: num(p.marketCap), liquidity: num((p.liquidity as Rec)?.usd),
            volume24h: num(vol?.h24), volume1h: num(vol?.h1), change24h: num(pc?.h24),
            buys24h: num(t24?.buys), sells24h: num(t24?.sells),
            buys1h: num(t1?.buys), sells1h: num(t1?.sells),
            holders: null, curve: null, graduated: false,
          });
        }
      }
...
function fireSignals(s: Snap): { kind: string; strength: string; title: string }[] {
  ...
  if (burst && (s.buys1h > s.sells1h * 2 || s.sells1h > s.buys1h * 2)) {
    ...
    out.push({ kind: "WHALE", strength: buying ? "BULLISH" : "BEARISH", title: `Whale ${buying ? "accumulation" : "exit"} burst on $${s.symbol}` });
  } else if (avgClip >= 400 && Math.abs(flow) >= 0.25) {
    out.push({ kind: "WHALE", strength: flow > 0 ? "BULLISH" : "BEARISH", title: `Large ${flow > 0 ? "buyers" : "sellers"} active on $${s.symbol}` });
  }
  if (s.change24h >= 15) out.push({ kind: "MOMENTUM", strength: "BULLISH", title: `+${s.change24h.toFixed(1)}% in 24h on $${s.symbol}` });
  else if (s.change24h <= -15) out.push({ kind: "MOMENTUM", strength: "BEARISH", title: `${s.change24h.toFixed(1)}% drawdown on $${s.symbol}` });
  if (turnover >= 0.3) out.push({ kind: "VOLUME", strength: "BULLISH", title: `Turnover ${(turnover * 100).toFixed(0)}% of cap on $${s.symbol}` });
  return out;
}
...
Deno.serve(async () => {
  ...
  // 1 · snapshots
  await db.from("price_snapshots").insert(feed.map((s) => ({
    ca: s.ca, price_usd: s.price, mcap: s.mcap, liquidity: s.liquidity,
    volume24h: s.volume24h, holder_count: s.holders, bonding_curve: s.curve,
  })));
  ...
  const events = feed.flatMap((s) =>
    fireSignals(s)
      ...
      .map((e) => ({ ca: s.ca, symbol: s.symbol, kind: e.kind, strength: e.strength, title: e.title, price_at: s.price, mcap_at: s.mcap })),
  );
  ...
  for (const ev of open ?? []) {
    ...
    await db.from("signal_events").update({ resolved: true, price_24h: now.price, change_24h: change, hit }).eq("id", ev.id);
    resolved++;
  }
